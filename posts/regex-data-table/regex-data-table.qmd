...
```{r load-text-file, message=FALSE}
#| echo: false
#| 
library(readr)
library(stringr)
...
data <- data |> 
  mutate(exchange = str_extract(agreement_text, "Exchange of.*\\."), # Extract the exchange of notes date
         ...
         exchange_year_test = str_extract(exchange, "(?<=^Exchange of notes.*?)(\d{4})"),
...
# Write a regular expression that captures four numbers in a row.
# 
