...
feed <- tryCatch(
  read_xml("https://www.quantitativepeace.com/feed/"),
  error = function(e) NULL
)

if (is.null(feed)) {
  cat('<p style="color:gray;">Recent posts unavailable.</p>')
} else {
  items <- xml_find_all(feed, "//item")[1:3]

  cat('<p style="font-size: 0.8rem; color: #999; margin-bottom: 0.8rem;">',
      'Recent posts from <a href="https://www.quantitativepeace.com" target="_blank" rel="noopener">Quantitative Peace</a>',
      '</p>')
  ...
  for (item in items) {
    ...
    # Format date
    date_fmt <- tryCatch(
      format(as.Date(pubdate, format = "%a, %d %b %Y"), "%B %d, %Y"),
      error = function(e) pubdate
    )

    # Strip HTML tags from description and truncate
    desc_clean <- gsub("<[^>]+>", "", desc)
    snippet    <- paste0(substr(desc_clean, 1, 180), "…")
...
">
  <div style="font-size: 0.78rem; color: #888; margin-bottom: 0.25rem;">%s</div>
  <a href="%s" target="_blank" rel="noopener"
     style="font-weight: 600; font-size: 1rem; text-decoration: none; color: #1a1a2e;">%s</a>
  <p style="margin: 0.4rem 0 0; font-size: 0.88rem; color: #444; line-height: 1.5;">%s</p>
</div>', date_fmt, link, title, snippet))
  }
}
```
